...
function renderInspector() {
  ...
  if(selected?.type==="paper") {
    const {account,marks,positions,orders,backtests,valuation}=selected.data;
    $("#inspector-content").innerHTML=`<div class="inspector-section"><span class="eyebrow">FINOS · SIMULATION</span><h3>${esc(account.title)}</h3><p>Paper cash ${rupees(account.cash_paise)} · starting ${rupees(account.starting_cash_paise)}.</p><p>${valuation?`Marked equity ${rupees(valuation.equity_paise)} · realized ${rupees(valuation.realized_pnl_paise)} · unrealized ${rupees(valuation.unrealized_pnl_paise)}. Oldest mark: ${esc(valuation.as_of||"none")}.`:"A complete marked valuation is not available."}</p></div><div class="inspector-section"><h3>Positions</h3>${positions.map(p=>`<p>${esc(p.symbol)} · ${p.quantity} units · cost ${rupees(p.cost_paise)} · manual mark ${p.mark?rupees(p.mark.price_paise):"missing"}</p>`).join("")||"<p>No positions.</p>"}</div><div class="inspector-section"><h3>Manual marks</h3>${marks.slice(0,20).map(m=>`<p>${esc(m.symbol)} · ${rupees(m.price_paise)} · ${esc(m.as_of)}<br>${esc(m.source_url)}</p>`).join("")||"<p>No prices recorded.</p>"}</div><div class="inspector-section"><h3>Historical replays</h3>${backtests.map(b=>`<p>${esc(b.symbol)} · ending ${rupees(b.ending_equity_paise)} · ${b.trade_count} simulated fills<br>SHA-256 ${esc(b.sha256)}<br><button class="ghost" data-action="inspect-backtest" data-id="${esc(b.id)}" data-account="${esc(account.id)}">Inspect replay →</button></p>`).join("")||"<p>No historical replay yet.</p>"}</div><div class="inspector-section"><h3>Simulated fills</h3>${orders.slice(0,30).map(o=>`<p>${esc(o.side)} ${o.quantity} ${esc(o.symbol)} · ${rupees(o.price_paise)} each · fee ${rupees(o.fee_paise)} · realized ${rupees(o.realized_pnl_paise)}<br>${esc(o.id)} · ${esc(o.status)}</p>`).join("")||"<p>No paper orders.</p>"}</div>`;
    return;
  }
  if(selected?.type==="backtest") {
    const {receipt,receipt_sha256}=selected.data;
    $("#inspector-content").innerHTML=`<div class="inspector-section"><span class="eyebrow">FINOS · HISTORICAL REPLAY</span><h3>${esc(receipt.symbol)}</h3><p>${esc(receipt.method)}</p><p>Fast ${receipt.fast_window} / slow ${receipt.slow_window} · simulated fee ${rupees(receipt.fee_paise)} per fill.</p><p>Starting ${rupees(receipt.starting_cash_paise)} · ending ${rupees(receipt.ending_equity_paise)} · difference ${rupees(receipt.pnl_paise)}.</p><p>${esc(receipt.limitations)}</p><p>Inputs SHA-256 ${esc(receipt.inputs_sha256)}<br>Receipt SHA-256 ${esc(receipt_sha256)}</p></div><div class="inspector-section"><h3>Simulated fills</h3>${receipt.trades.map(t=>`<p>${esc(t.as_of)} · ${esc(t.side)} ${t.quantity} at ${rupees(t.price_paise)} · fee ${rupees(t.fee_paise)}<br>Mark ${esc(t.mark_id)}</p>`).join("")||"<p>No signals created a fill.</p>"}</div><div class="inspector-section"><h3>Source marks</h3>${receipt.marks.map(m=>`<p>${esc(m.as_of)} · ${rupees(m.price_paise)} · ${esc(m.source_url)}</p>`).join("")}</div>`;
    return;
  }
...
function reviewHandoff(id){
  ...
  openModal("Review cross-hub evidence","Accepting grants the receiving mission this evidence. A task explicitly marked for automatic dispatch may start immediately and consume runtime usage; a crash before dispatch is reconciled on restart.",
...
async function replayPaperMarks(id) {
  let detail;
  try {detail=await api(`/api/finance/paper-accounts/${id}`)}catch(error){return notify(error.message,true)}
  if(!detail.marks.length)return notify("Record a historical sequence of manual marks first.",true);
  const requestId=crypto.randomUUID();
  openModal("Replay historical marks","Compare two moving averages using only earlier marks; simulate fills at the next observed mark. At least slow window + 2 distinct dated marks are needed. Manual sources and prices are not verified, and no account trade is placed.",
    `<label class="field">Symbol<select name="symbol">${detail.marks.map(m=>`<option value="${esc(m.symbol)}">${esc(m.symbol)}</option>`).join("")}</select></label><label class="field">Fast window<input name="fastWindow" type="number" min="2" max="30" step="1" value="2" required></label><label class="field">Slow window<input name="slowWindow" type="number" min="3" max="100" step="1" value="3" required></label><label class="field">Simulated fee per fill (₹)<input name="fee" type="number" min="0" step="0.01" value="0" required></label>`,
    async data=>{const result=await api(`/api/finance/paper-accounts/${id}/backtests`,{method:"POST",body:JSON.stringify({symbol:data.get("symbol"),fastWindow:Number(data.get("fastWindow")),slowWindow:Number(data.get("slowWindow")),feePaise:Math.round(Number(data.get("fee"))*100),requestId,expectedVersion:detail.account.version})});state.inspected={type:"backtest",data:result.backtest}});
}
async function inspectPaperReplay(account,id) {
  try {state.inspected={type:"backtest",data:(await api(`/api/finance/paper-accounts/${account}/backtests/${id}`)).backtest};renderInspector()}
  catch(error){notify(error.message,true)}
}
...
document.addEventListener("click",async event=>{
  ...
  else if(type==="paper-replay")replayPaperMarks(id);
  else if(type==="inspect-backtest")inspectPaperReplay(action.dataset.account,id);
  ...
  else if(type==="import-vault"){try{const result=await api("/api/vault/import",{method:"POST",body:"{}"});await refresh();notify(`Vault edits imported: ${result.imported.length}; conflicts: ${result.conflicts.length+result.projectionConflicts.length}.`)}catch(error){notify(error.message,true)}}
});
