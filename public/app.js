...
function hubsPage() {
  ...
  const replyRuntimes=state.data.runtimes.filter(r=>r.ready&&["codex","opencode"].includes(r.id));
...
function launchRun(taskId) {
  ...
  openModal(`Run the assigned ${esc(runtime)} specialist`,`AGAS will prepare an isolated Git worktree, send approved hub/project notes, and record CLI events and changed files. This may consume ${esc(runtime)} usage. Inspect output before accepting it.`,
