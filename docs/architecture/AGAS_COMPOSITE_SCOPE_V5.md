...
| ADAMS | Execution control plane | Durable queue, restart recovery, audit and receipts. Codex and OpenCode Dev adapters plus OpenCode text runs and an acknowledged cross-hub receipt have process fixture coverage. Live authenticated provider runs, leases and automatic dispatch remain required. |
...
| Teamwork | CEO replies and two-runtime acknowledged handoff, permissions and durable approvals | Durable CEO reply route through Codex/OpenCode has scoped process fixture checks. Content OpenCode text run → owner review → accepted handoff → required Dev Codex run → reviewed file passes a real child-process fixture and survives restart. Live provider runs, automatic dispatch and effect approvals remain pending. |
